...
class Hotel extends Bookable
{
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        if(!empty($adults = $request->input('adults')) and $adults > $total_adult_select){
            return $this->sendError(__("Sorry, the number of guests exceed the room capacity. Please add the number of rooms or select a different room"));
        }
        ...
        foreach ($rooms as $room) {
            if ($room->isAvailableAt($filters)) {
                ...
                $res[] = [
                    ...
                    'weekend_price'   => $room->tmp_weekend_price ?? 0,
