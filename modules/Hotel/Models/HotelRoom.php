...
class HotelRoom extends Bookable
{
    ...
    public function isAvailableAt($filters = []){
        ...
        foreach ($period as $dt){
            $dayOfWeek = $dt->format('N'); // 1 (Senin) sampai 7 (Minggu)
            $price = $this->price;

            // Cek apakah hari ini adalah akhir pekan (Sabtu: 6, Minggu: 7)
            if ($dayOfWeek == 6 || $dayOfWeek == 7) {
                $price = $this->weekend_price != 0 || $this->weekend_price != null ? $this->weekend_price : $price; // Menambahkan harga weekend ke variabel tmp_weekend_price
            } else {
                $price = $price; // Harga weekday
            }
            ...
            $allDates[$dt->format('Y-m-d')] = [
                ...
                'total_price' => $price + $this->weekend_price
            ];
            $tmp_night++;
        }
        if(!empty($roomDates))
        {
            ...
            {
                if(!$row->active or !$row->number or !$row->price or !$row->weekend_price) return false;
                ...
                $allDates[date('Y-m-d',strtotime($row->start_date))] = [
                    ...
                    'total_price' => $row->price+$row->weekend_price
                ];
            }
        }
