...
//        event(new VendorLogPayment($booking));
        ...
        if($booking->pay_now > 0) {
            ...
            return $this->sendSuccess( [
                'url'=>$booking->getDetailUrl()
            ], __("Your booking has been processed successfully"));
        }
    ...
    {
        if (!is_enable_guest_checkout() && !Auth::check()) {
            return $this->sendError(__("You have to login in to do this"))->setStatusCode(401);
        }
        ...
        if (!is_enable_guest_checkout() && $booking->customer_id != Auth::id()) {
            abort(404);
        }
        ...
        // Tambahkan logika untuk menentukan tombol yang ditampilkan
        $data['show_payment_button'] = $booking->status !== 'paid' && $booking->gateway === 'midtrans';
        $data['snap_token'] = $booking->snap_midtrans; // Pastikan snap_token tersimpan di booking
    ...
    public function handlePushReservation(Request $request)
{
    ...
    $validatedData = $request->validate([
        'reservation_id' => 'required|string|unique:bravo_bookings,code',
    ...
    $guestNames = explode(' ', $validatedData['guest_name'], 2);
    $firstName = $guestNames[0];
    $lastName = $guestNames[1] ?? '';

    // Save reservation data in bravo_bookings
    $booking = Booking::create([
        'code'       => $validatedData['reservation_id'], // Unique reservation ID
        'object_id'  => $validatedData['hotel_code'],     // Maps to Tiket.com's hotel code
        'first_name' => $firstName,
        'last_name'  => $lastName,
        'start_date' => $validatedData['check_in'],
        'end_date'   => $validatedData['check_out'],
        'status'     => 'confirmed',                     // Default to confirmed
    ]);

    // Save room data in bravo_hotel_room_bookings using HotelRoomBooking
    \Modules\Hotel\Models\HotelRoomBooking::create([
        'booking_id' => $booking->id,
        'room_id'    => $validatedData['room_code'], // Tiket.com's room code
        'start_date' => $validatedData['check_in'],
        'end_date'   => $validatedData['check_out'],
    ]);
    ...
    Notification::route('mail', 'team@example.com')
        ->notify(new NewReservationNotification($validatedData));
