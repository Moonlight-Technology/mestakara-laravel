...
class Booking extends BaseModel
{
    ...
    public function generateCode($location,$item)
    {
        return $location.".".$item.".".date('Ymd_His');
    }

    public function save(array $options = [])
    {
        if (empty($this->code))
            $this->code = $this->generateCode($this->service->location->location_code,$this->service->slug);
    ...
    public static function getStatisticChartData($from, $to, $statuses = false, $customer_id = false, $vendor_id = false, $location_id = null, $hotel_id = null)
{
    // Fix ver 1.5.1
    if ($statuses) {
        $list_statuses = [];
    ...
    // Fetch total published rooms
$totalRoomsQuery = \DB::table('bravo_hotel_rooms')
->join('bravo_hotels', 'bravo_hotel_rooms.parent_id', '=', 'bravo_hotels.id')
->where('bravo_hotel_rooms.status', 'publish');

// Apply additional location filter if provided
if (!empty($location_id)) {
$totalRoomsQuery->where('bravo_hotels.location_id', $location_id); // Apply location filter
}

if (!empty($hotel_id)) {
    $totalRoomsQuery->where('bravo_hotels.id', $hotel_id); // Apply resort filter
}

// Get the count of rooms
$totalRooms = $totalRoomsQuery->count(); // This will return an integer, no need to call count() again

$data['detail']['total_rooms']['val'] = $totalRooms; // Assign the count to the data
    ...
    if ($statuses) {
        $sql_raw[] = "count(CASE WHEN `bravo_bookings`.`status` != 'draft' THEN `bravo_bookings`.`id` ELSE NULL END) AS total_booking";
        foreach ($statuses as $status) {
            if (!in_array($status, static::$notAcceptedStatus)) {
                $sql_raw[] = "count(CASE WHEN `bravo_bookings`.`status` = '{$status}' THEN `bravo_bookings`.`id` ELSE NULL END) AS {$status}";
            }
        }
    }
    ...
    if (($to - $from) / DAY_IN_SECONDS > 90) {
        ...
        for ($month = 1; $month <= 12; $month++) {
            ...
            $dataBooking = parent::selectRaw(implode(",", $sql_raw))
                ...
                ->whereBetween('bravo_bookings.created_at', [
                    $year . '-' . $month . '-01 00:00:00',
                    $year . '-' . $month . '-' . $day_last_month . ' 23:59:59'
                ])
                ->whereNotIn('bravo_bookings.status', static::$notAcceptedStatus);
            ...
            if (!empty($location_id)) {
                $dataBooking = $dataBooking->where('bravo_hotels.location_id', $location_id); // Apply location filter
            }
            if (!empty($hotel_id)) {
                $dataBooking = $dataBooking->where('bravo_hotels.id', $hotel_id);
            }

            $dataBooking = $dataBooking->first();
            $data['chart']['labels'][] = date("F", strtotime($year . "-" . $month . "-01"));
            $data['chart']['datasets'][0]['data'][] = $dataBooking->total_price ?? 0;
            $data['chart']['datasets'][1]['data'][] = $dataBooking->total_fees ?? 0;
            $data['chart']['datasets'][2]['data'][] = $dataBooking->total_commission ?? 0;
        ...
        for ($i = strtotime(date('Y-m-d', $from)); $i <= strtotime(date('Y-m-d 23:59:59', $to)); $i += HOUR_IN_SECONDS) {
            ...
            $totalBookings = $data['detail']['total_booking']['val']; // Already calculated from data loop
            if ($totalRooms > 0) {
                $occupancy = ($totalBookings / $totalRooms) * 100;
                $data['detail']['total_occupancy']['val'] = round($occupancy, 2) . "%"; // Format as percentage
            } else {
                $data['detail']['total_occupancy']['val'] = "0%"; // No rooms, no occupancy
            }
